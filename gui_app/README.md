...
- 2020-10-8 修复bug-----在PDD_run1.m中删除负压判断的相关语句。
- 2020-10-8 检查输出位置。(1)IN Model.m->line45->line101->NC_bfs3.m->line53->"disp('无水水池')"；(2)IN Model.m->line45->line106->"disp(['破损管网连通性检查完成，存在', num2str(isolated_node_num),'个孤立节点，孤立节点及其邻接管段已删除！'])"；
- 2020-10-9 生成无关的2进展文件，issue#2，应该是PDD_run1.m中缺少关闭管网语句导致的。PDD_run1.m->line50在增加“calllib('epanet2','ENclose'); %关闭计算”
# 附录：试验记录
### 试验1-蒙特卡罗抽样次数为5
#### 描述/步骤
1.运行start_toolkit.m
...
不对，是在PDD_run1.m中，如果不产生负压，则直接返回，导致的计算错误。而当产生破坏太多后，导致大部分节点被删除，所以可以连接的节点中不再有负压。

### 试验10-200次抽样模拟
#### 修改
在PDD_run1.m中删除关于负压判断的语句
#### 描述/步骤
...
顺利通过模拟[命令行记录](./试验/试验10)
在128次抽样中出现了“无负压”，“65个孤立节点”
的情况。但是依然顺利通过了模拟。
